...
- For manual theme checks, preview relevant current navigation modes with `?navigation_menu=sidebar` and `?navigation_menu=header_dropdown`. Verify current core before relying on the historically documented `legacy` option.
